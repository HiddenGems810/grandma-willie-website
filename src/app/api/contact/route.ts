...
function validatePayload(body: unknown): ContactPayload | null {
  ...
  return {
    ...
    preferredContact:
      typeof b.preferredContact === "string" ? sanitize(b.preferredContact) : "",
  };
}
...
function renderEmailHtml(payload: ContactPayload): string {
  ...
  const safePreferredContact = payload.preferredContact
    ...
    <div style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; background: #FFF3D7; padding: 32px; border-radius: 12px;">
      ...
      <table style="width: 100%; border-collapse: collapse;">
        <tr>
          ...
          <td style="padding: 10px 0; color: #140B07;">${safeName}</td>
        </tr>
        <tr style="background: rgba(0,0,0,0.03);">
          ...
          <td style="padding: 10px 0; color: #140B07;">${safeContact}</td>
        </tr>
        <tr>
          ...
          <td style="padding: 10px 0; color: #140B07;">${safeInquiryType}</td>
        </tr>
        ${
          ...
          <td style="padding: 10px 0; color: #140B07;">${safePreferredContact}</td>
        </tr>`
            : ""
        }
        <tr>
        ...
        <tr>
          <td colspan="2" style="padding: 12px 16px; background: white; border-radius: 8px; color: #140B07; line-height: 1.6; font-size: 15px;">${safeMessage}</td>
        </tr>
      </table>
...
export async function POST(req: Request) {
  ...
  const notifyEmail = process.env.NOTIFY_EMAIL ?? "Will46shelby@gmail.com";
